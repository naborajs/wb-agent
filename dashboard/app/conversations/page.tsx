...
export default function LiveInboxPage() {
  ...
  const filteredConversations = conversations.filter((c) => {
    ...
    const isGroup = Boolean(c.metadata_json?.is_group) || Boolean(c.channel_id?.includes("@g.us"));
    if (filterMode === "whatsapp" && (c.channel !== "whatsapp" || isSim || isGroup)) return false;
    if (filterMode === "groups" && !isGroup) return false;
        ...
        <div className="p-3 border-b border-[var(--ed-border)] space-y-2">
          ...
          <div className="flex gap-1 text-[11px] font-medium text-[var(--ed-text-muted)] flex-wrap">
            ...
            >
              ...
              WA Direct ({conversations.filter((c) => c.channel === "whatsapp" && !c.metadata_json?.is_simulation && !c.metadata_json?.is_group && !c.channel_id?.includes("@g.us")).length})
            </button>
            <button
            ...
            >
              👥 Groups ({conversations.filter((c) => c.metadata_json?.is_group || c.channel_id?.includes("@g.us")).length})
            </button>
            <button
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-[var(--ed-border)]">
          {filteredConversations.length === 0 ? (
            ...
            filteredConversations.map((c) => {
              ...
              const isConvGroup = Boolean(c.metadata_json?.is_group) || Boolean(c.channel_id?.includes("@g.us"));
              return (
                <div
                  key={c.id}
          ...
          <>
            ...
            <div className="min-h-14 px-3 sm:px-6 py-2.5 sm:py-0 border-b border-[var(--ed-border)] bg-[var(--ed-surface)] flex flex-wrap sm:flex-nowrap items-center justify-between gap-2 shrink-0">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-[10px] sm:text-xs text-[var(--ed-text-muted)] truncate">
                    {isCurrentConvGroup ? "WhatsApp Group: " : (activeConv.channel === "simulation" || activeConvDetail?.conversation?.is_simulation ? "Sandbox: " : "WhatsApp: ")}
                    {activeConv.channel_id}
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap sm:flex-nowrap shrink-0">
                ...
                {isCurrentConvGroup ? (
                  <span
                    title="AI auto-reply is disabled for WhatsApp groups to avoid group spam. Operator replies manually."
                    className="inline-flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-3.5 py-1.5 rounded-xl text-xs font-semibold bg-amber-500/15 text-amber-400 border border-amber-500/30"
                  >
                    👥 Operator Only (Group)
                  </span>
            ...
            {/* WhatsApp Group Warning Banner */}
            {isCurrentConvGroup && (
              <div className="bg-amber-500/10 border-b border-amber-500/20 px-4 py-2 text-xs flex items-center justify-between text-amber-400 shrink-0">
                <div className="flex items-center gap-2">
                  <span className="text-base shrink-0">👥</span>
                  <span>
                    <strong>WhatsApp Group Chat:</strong> AI auto-reply is disabled for groups to prevent spam. The system is waiting for your manual response. Replies typed below will dispatch to the group.
                  </span>
                </div>
                          ...
                          {isInbound ? (
                            isMsgSim ? (
                              ...
                              <>
                                ...
                                <span>{isCurrentConvGroup && msg.sender_id ? msg.sender_id : "Customer"}</span>
                              </>
                            )
                          ) : msg.sender_type === "human" ? (
                            <>
