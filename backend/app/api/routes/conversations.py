...
async def suggest_reply_for_conversation(
    ...
    res = await conv_svc.suggest_reply(
        conversation_id=conversation_id,
        instructions=req.instructions,
        tone=req.tone,
        sender_participant=req.sender_participant,
    )
    if not res.get("success"):
        raise HTTPException(status_code=400, detail=res.get("error", "Failed to suggest reply."))

    return SuggestReplyResponse(**res)
