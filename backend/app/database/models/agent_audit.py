...
class FridayProblemReport(Base, OrgScopedMixin, TimestampMixin):
    """
    Structured issue and capability gap report generated by or for Friday.
    Stores internal runtime exceptions, unhandled user requests (capability gaps),
    tool/action execution failures, and explicit operator-reported bugs ('report this problem').
    """
    __tablename__ = "friday_problem_reports"

    id = Column(String(64), primary_key=True, default=generate_uuid)
    report_code = Column(String(32), nullable=False, unique=True, index=True)
    category = Column(String(64), default="CAPABILITY_GAP", nullable=False, index=True)  # CAPABILITY_GAP, INTERNAL_ERROR, TOOL_FAILURE, USER_REPORTED, NAVIGATION_FAILURE
    ...
    current_path = Column(String(128), default="/", nullable=True)
    error_trace = Column(Text, nullable=True)
    context_data = Column(UniversalJSON, default=dict, nullable=False)
    suggested_fix = Column(Text, nullable=True)
    status = Column(String(32), default="open", nullable=False, index=True)  # open, in_review, resolved, dismissed
    resolved_at = Column(DateTime(timezone=True), nullable=True)
    resolved_by = Column(String(128), nullable=True)
    resolution_notes = Column(Text, nullable=True)

    __table_args__ = (
        Index("ix_friday_problem_status_created", "status", "created_at"),
        Index("ix_friday_problem_category", "category"),
    )
